...
async function main() {
      const now = new Date();
      const { hour, minute, year, month, day } = getChileParts(now);
  ...
  if (!tipo) {
          console.log(`Fuera de horario de servicio (hora Chile ${hour}:${String(minute).padStart(2, "0")}). Sin actividad que simular.`);
          process.exit(0);
  }
  ...
  if (toInsert === 0) {
          console.log(`Ritmo al dia (hora Chile ${hour}:${String(minute).padStart(2, "0")}, ${alreadyServed}/${totalUsers} servidos, progreso ${(progress * 100).toFixed(0)}%). Sin nuevos tickets este ciclo.`);
          process.exit(0);
  }
  ...
  console.log(`Se registraron ${rows.length} tickets (hora Chile ${hour}:${String(minute).padStart(2, "0")}, tipo=${tipo}, progreso=${(progress * 100).toFixed(0)}%, total hoy=${alreadyServed + rows.length}/${totalUsers}).`);
      process.exit(0);
}
